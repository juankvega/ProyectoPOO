...
import com.uniMagdalena.controlador.sede.SedeControladorEditar;
 ...
 * @author macbook
 */
public class VistaSedeEditar extends SubScene {
    ...
    private final SedeDto objSede;
    
    private Pane panelCuerpo;
    private final BorderPane panelPrincipal;
    
    
    public VistaSedeEditar(Stage ventanaPadre, BorderPane princ, Pane pane, double ancho, double alto, SedeDto objSedeExterna, int posicionArchivo)
    {
    ...
    private void pintarFormulario()
    {
        ...
        cajaNombre.setText(objSede.getNombreSede());
        ...
        switch(objSede.getCiudadSede()) {
            case "Barranquilla" -> cbmCiudad.getSelectionModel().select(1);
            case "Santa Marta" -> cbmCiudad.getSelectionModel().select(2);
            case "Valledupar" -> cbmCiudad.getSelectionModel().select(3);
            default -> cbmCiudad.getSelectionModel().select(0);
        }
        ...
        cajaUbicacion.setText(objSede.getUbicacionSede());
        ...
        cajaImagen.setText(objSede.getNombreImagenPublicoSede());
        ...
        miGrilla.add(imgPrevisualizar, 2, 1, 1, 4);   //antes el ultimo era 3
        } 
        else 
        {
        ...
        imgPorDefecto = Icono.obtenerIconoExterno(objSede.getNombreImagenPrivadoSede(), 150); 
        GridPane.setHalignment(imgPorDefecto, HPos.CENTER);
        GridPane.setValignment(imgPorDefecto, VPos.CENTER);
        ...
        if(objSede.getEs24horasSede()) {
            slideSwitch24horas.setEstado(true);
        } else {
            slideSwitch24horas.setEstado(false);
        }
        ...
        Button btnEditar = new Button("Actualizar sede");
        ...
        btnEditar.setOnAction((e)->{actualizarSede();});
        ...
        btnRegresar.setOnAction((ActionEvent e) -> {
            panelCuerpo = SedeControladorVentana.administrar(
    ...
    private void actualizarSede()
    {
        ...
        Integer codSede = objSede.getIdSede();
        String nomSede = cajaNombre.getText();
        String ciudSede = obtenerCiudadCombo();
